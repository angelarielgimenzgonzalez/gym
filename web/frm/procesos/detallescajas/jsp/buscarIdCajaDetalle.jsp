
<%@page import="controladores.FacturasControlador"%>
<%@page import="modelos.Facturas"%>
<%@page import="modelos.DetallesCajas"%>
<%@page import="controladores.DetallesCajasControlador"%>
<%@page import="controladores.CajasControlador"%>
<%@page import="modelos.Cajas"%>
<%@page import="org.json.simple.JSONObject"%>
<%@page import="java.sql.ResultSet"%>
<%
    int id_detallecaja = Integer.parseInt(request.getParameter("id_detallecaja"));
    
    String tipo = "error";
    String mensaje = "Datos no encontrados.";
    String nuevo = "true";

    DetallesCajas detallecaja = DetallesCajasControlador.buscarId(id_detallecaja);
    if (detallecaja != null) {
        tipo = "success";
        mensaje = "Datos encontrados.";
        nuevo = "false";
    } else {
        detallecaja = new DetallesCajas();
        detallecaja.setId_detallecaja(0);
        detallecaja.setImporte_caja(0);

        Cajas caja = new Cajas();
        caja.setId_caja(0);
        detallecaja.setCaja(caja);

        Facturas factura = new Facturas();
        factura.setId_factura(0);
        factura.setNumero_factura(0);
        detallecaja.setFactura(factura);

        //Pagos pago = new Pagos();
        //pago.setId_pago(0);
        //pago.setForma_pago("");
        //detallecaja.setPago(pago);
    }
    //int id_factura_factura = detallecaja.getFacturafactura().getId_factura_factura();
    //System.out.println("idfactu "+ id_factura_factura);
    //FacturaFacturas facturafactura = new FacturaFacturas();
    //FacturaFacturas facturafactura = FacturaFacturasControlador.buscarTotalfactura(id_factura_factura);
    //int total1 = facturafactura.getTotal();
    //System.out.println("total1 "+ total1);
    //int importe = detallecaja.getImporte();
    //int vuelto1 = importe - total1;

    JSONObject obj = new JSONObject();
    obj.put("tipo", tipo);
    obj.put("mensaje", mensaje);
    obj.put("nuevo", nuevo);

    obj.put("id_detallecaja", String.valueOf(detallecaja.getId_detallecaja()));
    obj.put("id_caja", String.valueOf(detallecaja.getCaja().getId_caja()));
    System.out.println("cajaid= "+String.valueOf(detallecaja.getCaja().getId_caja()));
    obj.put("id_factura", String.valueOf(detallecaja.getFactura().getId_factura()));
    //obj.put("id_pago", String.valueOf(detallecaja.getPago().getId_pago()));
    obj.put("numero_factura", String.valueOf(detallecaja.getFactura().getNumero_factura()));
    obj.put("importe_caja", String.valueOf(detallecaja.getImporte_caja()));
    //obj.put("vuelto", (vuelto1));
    //obj.put("total", facturafactura.getTotal());
    //System.out.println("total "+ facturafactura.getTotal());
    //System.out.println("vuelto "+ vuelto1);
    out.print(obj);
    out.flush();
%>
